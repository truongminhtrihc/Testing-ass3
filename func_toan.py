...
def login(name, passw):
    try:
        username = WebDriverWait(driver, 20, 2).until(
            EC.presence_of_element_located((By.NAME, "username")))
        password = WebDriverWait(driver, 20, 2).until(
...
def accessToDashboard():
    try:
        dashboard_link = WebDriverWait(driver, 20, 2).until(EC.element_to_be_clickable(
